...
library(dplyr)
df_2 <- df %>% 
          mutate(cluster_id = if_else(is.na(oslo_area),
            paste(enumeration_area,locality_code, sep = ""),
            paste(enumeration_area,locality_code,oslo_area, sep = ""))) %>%
...
df <- df %>% 
  mutate(childdropout_check = case_when( coping_children_dropout == "yes" & (tot_children == 0 | dropout_num == 0) ~ "Flagged", TRUE~ ""))
...
df <- df %>% 
  mutate(childlabour_check = case_when( coping_child_labour == "yes" & (tot_children == 0 | under_18_working == "no" ) ~ "Flagged", TRUE~ "")) 
...
df <- df %>%
      mutate(tent_check = case_when(df$shelter_type == "tent" & (shelter_issues !="none")~ "Flagged", TRUE ~ "" ))
```
```{r}
df12 <- df %>% 
...
df <- df %>% 
    mutate(gender_check = case_when(df$gender_respondent == df$gender_respondent ~ "Flagged", TRUE ~ ""))
```
...
df <- df %>% 
      mutate(camp_check = case_when(df$camp == "yes" &  (df$hh_location_gaza != "shati_camp"| df$hh_location_gaza !="bureij_camp" | df$hh_location_gaza != "deir_balah_camp" | df$hh_location_gaza !="jabalya_camp" | df$hh_location_gaza != "khan_yunis_camp"| df$hh_location_gaza != "maghazi_camp"| df$hh_location_gaza != "nuseirat_camp" | df$hh_location_gaza != "rafah_camp" )~ "Flagged", TRUE ~ ""))
```
```{r}
df19 <- df %>% 
...
df <- df %>% 
      mutate(remote_learning_check = case_when(remote_learning == dropout_num ~ "Flagged", TRUE ~ ""))
```
```{r}
df24<- df %>% 
...
```{r}
##Flag if tot_cwd == 0 and drop_out_reason == "Disability"
df <- df %>% 
      mutate(disability_check = case_when( tot_cwd == 0 & drop_out_reason == "Disability" ~ "Flagged", TRUE ~ ""))
```

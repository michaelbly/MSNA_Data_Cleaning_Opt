...
westbank_sample_inperson <- westbank_sample_df_summarized %>% 
                          dplyr::select(strata, Sample = num_surveys)
...
westbank_tracking_merged_in_person <- rbind(westbank_tracking_merged_in_person, sumrow_in_person ) %>%
  mutate_all(
    funs(replace_na(.,0)))
westbank_tracking_merged_in_person
########################################################################################
...
survey_length <- df %>% 
  ...
  dplyr::summarise(
    Median_duration = round(median(as.numeric(df$interview_duration), na.rm = T),0)
  ) %>% 
...
short_interviews_by_enum <- df %>% 
  filter(df$consent == "yes" ) %>% 
  dplyr::filter(interview_duration < time_limit & today > "2021-06-16") %>% 
